recommended products in cart pages before checkout.
Generate a session secret using OpenSSL:-
   openssl rand -hex 32
...
export class MemStorage implements IStorage {
  ...
  private loadFromFile() {
    try {
      ...
      this.products = new Map(parsed.products.map((p: Product) => [p.id, p]));
      this.orders = new Map(parsed.orders.map((o: Order) => [o.id, o]));
      this.addresses = new Map(parsed.addresses.map((a: Address) => [a.id, a]));
      
      this.currentUserId = parsed.currentUserId;
      this.currentProductId = parsed.currentProductId;
      this.currentOrderId = parsed.currentOrderId;
      this.currentAddressId = parsed.currentAddressId;
    } catch (error) {
  ...
  async deleteAddress(id: number): Promise<boolean> {
    ...
    const result = this.addresses.delete(id);
    this.saveToFile();
    return result;
  }
